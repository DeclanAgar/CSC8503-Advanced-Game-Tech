	...
	for (auto i = first; i != last; ++i) {
		...
		for (auto j = i + 1; j != last; ++j) {
			...
			if (CollisionDetection::ObjectIntersection(*i, *j, info)) {
				...
				if (info.a->GetName() == "Player" && info.b->GetName() == "Sphere") {
					allCollisions.erase(info);
					gameWorld.RemoveGameObject(info.b, true);
					gameWorld.IncrementScore();
					return;
				}
				else if (info.a->GetName() == "Player" && info.b->GetName() == "Apple") {
					allCollisions.erase(info);
					gameWorld.RemoveGameObject(info.b, true);
					gameWorld.ToggleSpeedBoost();
					return;
				}
				else {
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	float cRestitution = a.GetcRestitution(); // Disperse some kinetic  energy || Cubes and Spheres have different coefficients
		...
		tree.OperateOnContents([&](std::list<QuadTreeEntry<GameObject*>>& data) {
			...
			for (auto i = data.begin(); i != data.end(); ++i) {
				for (auto j = std::next(i); j != data.end(); ++j) {
					...
					// If the same pair is in another quadtree together etc
...
void PhysicsSystem::IntegrateAccel(float dt) {
	...
	for (auto i = first; i != last; ++i) {
		...
		if (object == nullptr) {
			continue; // Game object has no physics object
		}
